...
HCT_U_items <- HCT_items[11:15]
HCT_neg     <- c("hct_r03","hct_t02","hct_t05","hct_u03","hct_u05")
...
scale_min <- min(raw[HCT_items], na.rm = TRUE)
scale_max <- max(raw[HCT_items], na.rm = TRUE)
rev_fun   <- \(x) ifelse(is.na(x), NA, scale_max + scale_min - x)

# Filter + Scores -----------------------------------------------------
data <- raw |>
  filter(age >= 18) |>
  mutate(across(all_of(HCT_neg), rev_fun)) |>
  mutate(
    HCT       = rowMeans(pick(all_of(HCT_items)), na.rm = TRUE),
    HCT_U     = rowMeans(pick(all_of(HCT_U_items)), na.rm = TRUE),
...
```{r}
#| label: fig-scat
#| fig-cap: "Zusammenhang Understandability und Gesamtvertrauen"
